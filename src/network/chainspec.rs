...
pub fn bsc_chain_spec() -> Arc<ChainSpec> {
    let genesis = serde_json::from_str(include_str!("../../data/genesis.json"))
    ...
    ChainSpec {
        ...
        genesis: serde_json::from_str(include_str!("../../data/genesis.json"))
            .expect("Can't deserialize BSC Mainnet genesis json"),
...
pub fn bootnodes() -> Vec<NodeRecord> {
    // BOOTNODES[..].iter().map(|s| s.parse().unwrap()).collect()
    parse_nodes(BOOTNODES)
}
pub fn head() -> Head {
    Head {
        number: 40_000_000,
