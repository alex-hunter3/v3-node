...
//! geth.
use alloy::rlp::{Decodable, Encodable, RlpDecodable, RlpEncodable};
use libp2p::bytes::{Buf, BufMut, Bytes, BytesMut};
...
impl Decodable for UpgradeStatus {
    fn decode(buf: &mut &[u8]) -> alloy::rlp::Result<Self> {
        ...
        if message_id != UPGRADE_STATUS_MESSAGE_ID {
            return Err(alloy::rlp::Error::Custom("Invalid message ID"));
        }
        buf.advance(1);
